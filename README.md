...
Ambas APIs aceptan tambien un token compartido (bearer) para llamadas internas:
...
Si ves `EADDRINUSE` en el puerto `3002`, significa que ya hay otro proceso escuchando ahí.
...
También puedes ver los archivos YAML directamente:
- `customers-api/openapi.yaml`
- `orders-api/openapi.yaml`

## Errores típicos

- **Llamar al puerto equivocado**:
  - Customers API es `3001`
  - Orders API es `3002`
  - Orquestador es `3003`
- **`Cannot GET /customers` en HTML**: normalmente estás pegándole al orquestador (`3003`) por error. El orquestador no sirve `/customers`.
- **401 Unauthorized**:
  - Customers/Orders necesitan `Authorization: Bearer <JWT>` o `Bearer <SERVICE_TOKEN>`
  - El orquestador necesita `Authorization: Bearer <ORCHESTRATOR_TOKEN>` si esa variable está definida
